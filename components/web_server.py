...
class WebServer:
    def __init__(self, documents, errors):
        ...
        self.web_server.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        self.web_server.bind((self.address, port))
        ...
        if documents_list:
            ...
            for document in documents_list:
                if document.split(".")[0] != 'favicon':
                    crated_index += f' <li><a href="http://localhost:{port}/{file}{document}"' \
        ...
        while True:
            ...
            if http_message:
                try:
                    ...
                    # forbidden error
                    elif specifications.file == '/admin':
                        self.handle_error(socket_client, 403)
                        print('error 403\n')
